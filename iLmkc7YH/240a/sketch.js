...
function draw() {
    if(isDraw) {
        ...
        if(freeze<1) {
            ...
            if(stateA < -maxA) { // max heading angle
                stateA = -maxA;
            } else if(stateA > maxA) {
                stateA = maxA;
            } if(angle[0] < -maxA) { // falls over
                angle[0] = -maxA;
                freeze = freeze_time;
                freeze_mode = 2;
            ...
            angle[3] = fixBetween(angle[3],-maxA/4,maxA/4);
...
function startGame() {
    ...
    A_mat = [[1.0,0.0,0.01,0],[0,1,0,0.01],[0.1,-0.15,0.985,0.02],[0.002,-0.2,0.005,0.98]];
