...
function draw() {
    if(isDraw) {
        ...
        if(perturbation>0) {
            if(perturbing > 0) {
                /*if(-dotY<perturbCoord[nextPerturb]+perturbLen) {
                ...
                if(nextPerturb<perturbCount && -dotY>perturbCoord[nextPerturb]) {
                    console.log(perturbDir[nextPerturb]+" "+dotY+" "+nextPerturb+" "+perturbCoord[nextPerturb]);
...
function collision() { // handles collision, returns dist to path
    var minDist = 4 * maxX*maxX;
    var minVec = null;
    let startFix = int(-dotY - pathWidth*100);
    if(startFix < 0)
    ...
    if(endFix > lines.length)
        endFix = lines.length;
    for(let i = startFix+1; i<endFix; i++) {
        let proj = projToSegmentSquared({x: dotX, y: -dotY}, {x: lines[i-1], y: (i-1)}, {x: lines[i], y:i});
        let dist = dist2({x: dotX, y: -dotY}, proj);
        if(dist < minDist) {
            minDist = dist;
            minVec = proj;
        }
    }
    //console.log("x: "+dotX+", y: "+dotY);
    if(minDist > pathWidth**2) {
        dotX = minVec.x + (dotX - minVec.x)*pathWidth/sqrt(minDist);
        dotY = -minVec.y - (-dotY - minVec.y)*pathWidth/sqrt(minDist);
    }
    return minDist;
}
function sinuousCurve(len) {
    ...
    /*for(let i=0; i<straightLen; i++) {
        points.push(0.0);
        lPath.push({x: -1*pathWidth, y: i}); //
        rPath.push({x: 1*pathWidth, y: i}); //
    }*/
    for(let i=0; i<len-straightLen; i++) {
        var X = 0;
        var X2 = 0;
        for(let j=0; j<max_amplitudes.length; j++) {
            /*if(i<straightLen) {
                X += width_x/2*i/straightLen*max_amplitudes[j]*sin(2*PI*start*frequency[j]);
                X2 += 2*PI*frequency[j]*0.01*width_x/2*i/straightLen*max_amplitudes[j]*cos(2*PI*start*frequency[j]);
            } else {
    ...
    for(let i=len-straightLen; i<len; i++) {
        points.push(X);
        lPath.push({x: -1*pathWidth + X, y: i}); //
        rPath.push({x: 1*pathWidth + X, y: i}); //
    }
    path.push(lPath); //
    path.push(rPath); //
    return points;
...
/*function drawBike() {
    ...
    line(x, y, x-40*sin(heading), y+40*cos(heading));
    let A = dotU*20;
    line(x-20*cos(heading+A), y-20*sin(heading+A), x+20*cos(heading+A), y+20*sin(heading+A));
    strokeWeight(4);
    line(x-10*sin(heading+A), y+10*cos(heading+A), x+10*sin(heading+A), y-10*cos(heading+A));
    //triangle(x, y, x-10*sin(heading)+4*cos(heading), y+10*cos(heading)+4*sin(heading), x-10*sin(heading)-4*cos(heading), y+10*cos(heading)-4*sin(heading));
}*/
function drawTrace() {
    var baseColor;
