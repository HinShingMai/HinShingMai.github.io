                ...
                if(-dotY+30*cos(heading)/scaling >= plen) {
                    ...
                    if(mode == 0 || mode == 5) {
                        ...
                        if(scores.length<5) { // less than 5 trials, no feedback
                            ...
                            let errDiff = (mean_error-meanStd2[0])/meanStd2[1];
                            let accZ = 2-errDiff*2;
                            score_max = fixBetween(5+accZ*15, 0, 100).toFixed(0);
                            feedback_sc = fixBetween(Math.floor(accZ), 0, 5);
                        }
                        /*else {
                            movin = -120;
                        ...
                        if(score_base[0].length < 21 && mode == 0 && blockType == 1) { // save speed and error for bad feedback during learning blocks
                            score_base[0].push(mean_speed);
                            score_base[1].push(mean_error);
                        }
                    } else
