...
var amplitudes_select = [[[6.93*10,4.03*0.73/2.27*1.30*1.30*10, 4.03*0.73/2.27*0.97*0.97*10], [6.93*15,3.42*0.73/1.84*1.03*1.03*15, 3.42*0.73/1.84*0.81*0.81*15]],
                            [[2.31*16, 2.31*8, 2.31*8, 1.76*4, 1.30*4, 0.97*2], [2.31*16, 2.31*8, 2.31*8, 1.58*4, 1.03*4, 0.81*2]],
                            [[2.31*16, 2.31*8, 2.31*8, 1.76*4, 1.30*4, 0.97*2], [2.31*16, 2.31*8, 2.31*8, 1.58*4, 1.03*4, 0.81*2]]];
var frequency_select = [[[0.1,1.15, 1.55], [0.15,1.45, 1.85]],
                            [[0.1*3, 0.25*3, 0.55*3, 0.85*3, 1.15*3, 1.55*3], [0.1*3, 0.35*3, 0.65*3, 0.95*3, 1.45*3, 1.85*3]],
                            [[0.1*3, 0.25*3, 0.55*3, 0.85*3, 1.15*3, 1.55*3], [0.1*3, 0.25*3, 0.55*3, 0.85*3, 1.15*3, 1.55*3]]];
let amplitudes;
let frequency;
...
function draw() {
    if(isDraw) {
        if(frameNum >= maxPoints+maxTailLen) {
            ...
            let mean_err = trimmedAverage(error, 0.01);// error/frameNum;
        ...
        if(touchState[0] && touchState[1]) {//(touchState[0] && touchState[1]) {
            if(freeze<1) {
                // record trajectory
                dis_temp.push([dotX, dotY]);
                //vDist_temp.push(dotY);
            ...
            //erBuffer = 'Touch both button on both sides to start !';
            //backcolor = 'lighgray';
        }
        dotU = [0,0]; // 0,0
        // draw
        ...
        for(let i=1; i<len+maxTailLen; i++) {
            while(dis < speed) { // add next point of SPEED distance away
                ...
                for(let j=0; j<ampl[0].length; j++) {
                    post_pt[0] += ampl[0][j]*sin(2*PI*start*freq[0][j]+phase[k][0][j]);
                    post_pt[1] += ampl[1][j]*cos(2*PI*start*freq[1][j]+phase[k][1][j]);
                }
...
function drawOverlay() {
    if(!touchState[0] || !touchState[1]) {
        ...
        text("FPS: "+frBuffer, maxX*scaling-30, -maxY*scaling+10);
        textSize(Math.floor(10*scaling));
        text(fbMsg+"\n\nTouch the button on both sides to start!", 0, 0);
