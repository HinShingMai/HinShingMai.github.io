...
let sign_choice;
let horiSteps = 3;
let weig = [1/horiSteps, 1/horiSteps, 1/horiSteps];
        ...
        if(movin>0) {
            if(delay == -1) {
                ...
                if(nextCrossing >= horiSteps) {
                    ...
                    console.log(sc_er);
                    if(mode == 0) {
                        //let mean_speed = SAT1_score[1]/SAT1_score[2]; // SAT_score = [sum of speed, sum of error, n]
                        movin = -120;
                        ...
                        //let mean_speed = SAT1_score[1]/SAT1_score[2];
                        movin = -60;
                        ding[1].play();
                    }
                    blanknum++;
...
function getScoreError(crossing_p, crossing_t, weight) {
    var sumError = 0;
    for(let i=0; i<crossing_p.length; i++)
        sumError += weight[i]*Math.abs(crossing_t[i].x-crossing_p[i]);
    return [Math.max(1 - sumError/maxX, 0), sumError];
}
...
function randomLine(steps, num) {
    ...
    for(let j=0; j<num; j++) {
        ...
        for(let i=0; i<steps; i++)
            point.push({x: random(-1,1)*maxX, y:(i+1)*YDist})
        points.push(point);
    }
    return points;
}
...
function drawReturnCursor() {
    ...
    if(movin==0) {
        ...
        if(blanknum<1) {
            ...
            rect(-maxX*scaling, sMargin*scaling, maxX*scaling*2, -wHeight*scaling);//
            drawGoal(lines[blank[blanknum-1]]);
            if(mode != 0 && mode != 5)
                drawCurve(lines[blank[blanknum-1]]);
            if(mode == 1)
                drawTrace(true);
            if(mode == 0) {
                strokeWeight(1);
