...
function startSession() {
    ...
    if(sessionsType[currentSession] == 0) { // familiarization session
        if(currentTrainBlock==0) {
            ...
            mode = 2; // 0: normal, 1: familiarization, 2/3: mouse calibration, 4: no-feedback straight, 5: no-feedback trial, 6: traj feedback trial
        ...
        if(sessionsType[currentSession] > 2) { // baseline block
            modes = Array(20).fill(5);
            reflect -= 2;
            gainX = 1;
        } else {
            if(currentSession > trainBlocks.length-3) // retention or generalization
                ...
                modes = pesudoRandom(2, 10, 1, 0, 6, true); // Array(40).fill(0);
            gainX = 2;
        }
        //modes = Array(40).fill(0);
        movin = -300;
        lines = sinuousCurve(maxPoints, reflect);
    }
    blank = modes.length;
    clear();
    dis_instr = 1;
