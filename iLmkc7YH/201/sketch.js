...
function draw() {
    if(isDraw) {
        ...
        if(offset == 0)
            ...
            dotU = fixBetween(-angAcc, -maxA, +maxA);
        //dotA = dotA + dotU + moveNoise(blockType); 
        dotA = fixBetween(dotA + dotU + moveNoise(blockType), -maxA, +maxA);
        dotX = fixBetween(dotX + dotV[0]*sin(dotA), -maxX, maxX); //fixBetween(dotX + dotV[0]*dotA, -maxX, maxX);
        dotY -= dotV[1]*cos(dotA); //dotV[1];
...
function drawCurve(coords, start, end) {
    ...
    /*stroke('lightgrey');
    strokeWeight(sqrt(error/min(frameNum+1, maxPoints))*scaling*scaling_x);
    for(let i = startFix+1; i<endFix; i++)
