...
let trainBlocks = [0,3,-1,1,-1,1,-10,4];//[0,3,1,4];
/*
-n: n-minutes break
    ...
    if(sessionsType[currentSession] == 0) { // familiarization session
        ...
        if(sessionsType[currentSession] > 2) { // baseline or retention block
            ...
            if(sessionsType[currentSession] == 3) { // baseline
                dis_instr = 2;
                blank = Array.from([0,1,4,5,8,9,12,13].flatMap(i => [i,i,i])).sort(() => Math.random()-0.5);
                //blank = [0,1,4,5,8,9,12,13];
        ...
        //modes = Array(40).fill(0);
        movin = -300;
        lines = sinuousCurve(maxPoints, blank);
    }
    clear();
    blanknum = 0;
    frameNum = 0;
    ...
    if(isDraw) {
        if(movin>0) {
            if(delay == -1) {
                ...
                //nse.push(noise);
                var inPath = true;
                var pathError = distToPath(mode != 6); //distToPath(true);
                ...
                if(-dotY+30*cos(heading)/scaling >= maxPoints) {
                    ...
                    if(mode == 0 || mode == 6) {
                        ...
                        movin = -120;
                        let score = getScore(mean_error, err_func);
                        scores.push(score);
...
function getScore(error, mode) {
    if(mode == 1)
        return 200/(error+200);
    else if(mode == 2)
        return 400/(error+400);
    else if(mode == 3)
        return 900/(error+900);
    else if(mode == 4)
        return 1600/(error+1600);
    else
        return error;
}
...
function distToPath(mirror=false) { // squared distance to path, approximate
    var minDist = 4 * maxX*maxX;
    let startFix = int(-dotY - pathWidth*100);
    if(startFix < 0)
        startFix = 0;
    let endFix = int(-dotY + pathWidth*100);
    if(endFix > lines[blanknum].length)
        endFix = lines[blanknum].length;
    if(mirror) { // distance to mirrored path
        for(let i = startFix+1; i<endFix; i++) {
            let dist = distToSegmentSquared({x: dotX, y: -dotY}, {x: -lines[blanknum][i-1], y: (i-1)}, {x: -lines[blanknum][i], y:i});
            if(dist < minDist)
                minDist = dist;
        }
    } else { // distance to actual path
        for(let i = startFix+1; i<endFix; i++) {
            let dist = distToSegmentSquared({x: dotX, y: -dotY}, {x: lines[blanknum][i-1], y: (i-1)}, {x: lines[blanknum][i], y:i});
            if(dist < minDist)
                minDist = dist;
...
function linearError() { // horizontal distance to trajectory
    if(lines==null || dotY>0 || -dotY>=lines[blanknum].length+1)
        return 0;
    return (dotX-lines[blanknum][Math.floor(-dotY)])**2;
}
...
function sinuousCurve(len, c) { // len: total length of trajectory, c: traj type
    const wid = 1/1024;
    const amp = width_x/4;
    var X, X2;
    var repeat = c.length;
    ...
    for(let k=0; k<repeat; k++) { // generate each sub-trajectory
        ...
        } else if(c[k] < 12) { // symmetric S curve
            let amp_s = amp*sig[0]/1.3;
            for(let i=0; i<len; i++) {
...
function drawInstr() {
    stroke('blue');
    fill('blue');
    rect(lines[blanknum][0]-30*scaling, 0, 60*scaling, 20*scaling);
    fill('white');
    stroke('white');
