...
function drawCurve(coords, framenum) {
    if(coords!==null) {
        ...
        if(time-2 > start) {
            ...
            //line(coords[time-2][0]*scaling, -coords[time-2][1]*scaling, (coords[time-1][0]+(coords[time-1][0]-coords[time-2][0])*10)*scaling, -(coords[time-1][1]+(coords[time-1][1]-coords[time-2][1])*10)*scaling);
        }
        noStroke();
        fill('green');
        for(let i = start+1; i<time; i++) {
...
function drawTrace() { // draw trace behind triangle
    ...
    for(let i in trace) {
        ...
        stroke(baseColor);
        fill(baseColor);
        ellipse(trace[i].x*scaling, -trace[i].y*scaling, 1, 1);
