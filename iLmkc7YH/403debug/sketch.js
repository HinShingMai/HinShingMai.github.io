...
function startSession() {
    ...
    if(sessionsType[currentSession] == 0) { // familiarization session
        if(currentTrainBlock==0) {
            ...
            modes = Array(1).fill(1);// Array(1).fill(1).concat(Array(1).fill(4));
    ...
    if(isDraw) {
        if(movin>0) {
            if(delay == -1) {
                ...
                //nse.push(noise);
                var inPath = true;
                var pathError = distToPath(err_func==4? 3:2);
                ...
                if(-dotY+30*cos(heading)/scaling >= maxPoints) {
                    ...
                    if(mode == 0 || mode == 6) {
                        ...
                        movin = -120;
                        let score = getScore(mean_error, err_func); //200/(mean_error+200);
                        scores.push(score);
...
function distToPath(ln=2) { // Ln distance to path, approximate. squared distance if n=2
...
function drawInstr() {
    stroke('blue');
    fill('blue');
    rect(-30*scaling, 0, 60*scaling, 20*scaling);
    fill('white');
    stroke('white');
