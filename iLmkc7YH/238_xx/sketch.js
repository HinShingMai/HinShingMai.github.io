...
function drawBox3() { // draws a box with the dimensions, displacements and rotations specified.
    ...
    let z_rot = dotA;
    // conjugating
    let conj = (x) => [x[0],-x[1],-x[2],-x[3]];
    // multiplying two quaternions
    let mult = (x, y) => [x[0]*y[0]-x[1]*y[1]-x[2]*y[2]-x[3]*y[3], x[0]*y[1]+x[1]*y[0]+x[2]*y[3]-x[3]*y[2], x[0]*y[2]-x[1]*y[3]+x[2]*y[0]+x[3]*y[1], x[0]*y[3]+x[1]*y[2]-x[2]*y[1]+x[3]*y[0]];
    // rotation in 3d space by multiplying three rotation vectors
    let rota = (x,y,z) => mult(mult(x, y), z);
    // coordinate of vertices
    var v1 = [0, x_len, y_len, z_len];
    var v2 = [0, -x_len, y_len, z_len];
    var v3 = [0, -x_len, -y_len, z_len];
    var v4 = [0, x_len, -y_len, z_len];
    var v5 = [0, x_len, y_len, -z_len];
    var v6 = [0, -x_len, y_len, -z_len];
    var v7 = [0, -x_len, -y_len, -z_len];
    var v8 = [0, x_len, -y_len, -z_len];
    // rotation vectors
    var xquat = [cos(x_rot/2), sin(x_rot/2), 0, 0];
    var yquat = [cos(y_rot/2), 0, sin(y_rot/2), 0];
    var zquat = [cos(z_rot/2), 0, 0, sin(z_rot/2)];
    var xconj = conj(xquat);
    var yconj = conj(yquat);
    var zconj = conj(zquat);
    // coordinates of vertices post rotation
    var v1r = rota(zquat,rota(yquat,rota(xquat,v1,xconj),yconj),zconj);
    var v2r = rota(zquat,rota(yquat,rota(xquat,v2,xconj),yconj),zconj);
    var v3r = rota(zquat,rota(yquat,rota(xquat,v3,xconj),yconj),zconj);
    var v4r = rota(zquat,rota(yquat,rota(xquat,v4,xconj),yconj),zconj);
    var v5r = rota(zquat,rota(yquat,rota(xquat,v5,xconj),yconj),zconj);
    var v6r = rota(zquat,rota(yquat,rota(xquat,v6,xconj),yconj),zconj);
    var v7r = rota(zquat,rota(yquat,rota(xquat,v7,xconj),yconj),zconj);
    var v8r = rota(zquat,rota(yquat,rota(xquat,v8,xconj),yconj),zconj);
    // draw faces
    push();
    translate(x_dis, y_dis);
    stroke('blue');
    fill('blue');
    quad(v5r[1],v5r[2],v6r[1],v6r[2],v7r[1],v7r[2],v8r[1],v8r[2]);
    quad(v1r[1],v1r[2],v2r[1],v2r[2],v3r[1],v3r[2],v4r[1],v4r[2]);
    quad(v1r[1],v1r[2],v5r[1],v5r[2],v8r[1],v8r[2],v4r[1],v4r[2]);
    quad(v1r[1],v1r[2],v2r[1],v2r[2],v6r[1],v6r[2],v5r[1],v5r[2]);
    quad(v2r[1],v2r[2],v3r[1],v3r[2],v7r[1],v7r[2],v6r[1],v6r[2]);
    quad(v3r[1],v3r[2],v4r[1],v4r[2],v8r[1],v8r[2],v7r[1],v7r[2]);
    pop();
}
function drawBike() {
    if(!movin)
