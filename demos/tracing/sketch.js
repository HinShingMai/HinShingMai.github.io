...
function startSession() {
    ...
    if(sessionsType[currentSession] == 0) { // familiarization session
        ...
        wHeight = maxY+2*sMargin;
        scaling = scaling_base;
        [lines_real, lines_show] = sinuousCurve(maxPoints, sessionsType[currentSession]);
        dotX = lines_real[0];
        dotY = -maxY/2;
        gainX = 1;
    }
...
function distToPath() { // squared distance to path, approximate
    ...
    if(endFix > lines_real.length)
        endFix = lines_real.length;
    for(let i = startFix+1; i<endFix; i++) {
        let dist = distToSegmentSquared({x: dotX, y: -dotY}, {x: lines_real[i-1], y: (i-1)}, {x: lines_real[i], y:i});
        if(dist < minDist)
            minDist = dist;
    }
    return minDist;
...
function linearError() { // horizontal distance to trajectory
    if(lines_real==null || dotY>0 || -dotY>=lines_real.length+1)
        return 0;
    return (dotX-lines_real[Math.floor(-dotY)])**2;
}
...
function drawReturnCursor() {
    ...
    if(movin==0) {
        ...
        if(delay<0) {
            ...
            rect(lines_real[0]-30*scaling, 0, 60*scaling, 20*scaling);
...
function handleMouseMove(e) {
    if(movin>0) {
        if(delay<0) {
            var scaledMovementX = e.movementX/speed_scale/gainX;
            var scaledMovementY = e.movementY/speed_scale;
            dotA += scaledMovementX;
            dotB += scaledMovementY;
        }
    } else {
        dotX = fixBetween(dotX+e.movementX/speed_scale/gainX,-maxX,maxX);
